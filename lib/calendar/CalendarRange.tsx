...
function CalendarRange(originalProps: CalendarRangeProps) {
  ...
  const handlePick = (date: Date, type: string) => {
    ...
    if (isValidDate(startValue) && !isValidDate(endValue)) {
      ...
      props.selectone?.(innerValue.value);
    }

    const diffDays = Math.ceil(
      Math.abs(innerValue.value[1].getTime() - innerValue.value[0].getTime()) /
        (1000 * 60 * 60 * 24)
    );
    props.selectRange?.(diffDays);
  };
...
export const calendarRangeProps = [...calendarProps, 'selectone', 'selectRange'];
