...
function Calendar(originalProps: CalendarProps) {
  ...
  const getDateClasses = (cellDate: Date) => {
    ...
    const dateCell = new Date(cellDate.getFullYear(), cellDate.getMonth(), cellDate.getDate());

    if (dateCell.getDate() === startOfMonth(dateCell).getDate()) {
      classes.push('start-month');
    }
    if (dateCell.getDate() === endOfMonth(dateCell).getDate()) {
  ...
  const getMonthClasses = (cellDate: Date) => {
    ...
    if (props.type !== 'month') {
      ...
      classes.push(innerCalendar.value.getMonth() === cellDate.getMonth() ? 'active' : '');
      return classes.join(' ');
    }
  ...
  const getYearClasses = (cellDate: Date) => {
    ...
    if (props.type !== 'year') {
      ...
      classes.push(innerCalendar.value.getFullYear() === cellDate.getFullYear() ? 'active' : '');
      return classes.join(' ');
    }
  ...
  return () => {
    ...
    return (
      <TableDate
        ...
        calendarTextFormat={props.calendarTextFormat}
        calendar={innerCalendar.value}
