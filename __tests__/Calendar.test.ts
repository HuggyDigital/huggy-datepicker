...
describe('CalendarPanel', () => {
  ...
  it('feat: click prev/next month', async () => {
    ...
    const getCurrentYear = () => parseInt(wrapper.find('.mx-btn-current-year').text(), 10);
    const getCurrentMonth = () => locale.months.indexOf(wrapper.find('.mx-btn-current-month').text());
    let count = 12;
  ...
  it('feat: select year to change the calendar', async () => {
    ...
    await wrapper.find('.mx-table-month td > div').trigger('click');
    expect(wrapper.find('.mx-btn-current-month').text()).toBe(locale.months[0]);
  });
